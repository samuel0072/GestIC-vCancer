...
class User {
    ...
    validateUserProfile = async ({ token, validProfileTags = [] }) => {
        try {
            const { id, profileId } = await jwt.verify(
                token,
                process.env.SECRET
            );
    ...
    register = async ({ name, email, password, profile, title }) => {
        try {
    ...
    getById = async ({ token, idInToken, id }) => {
        try {
            const isAdminProfile = await this.validateUserProfile({
                token,
                validProfileTags: allowedProfiles,
            });
    ...
    getUsers = async ({ token, status = undefined }) => {
        try {
    ...
    change_password = async ({ id, old_password, new_password }) => {
        try {
            let user = await db.user.findByPk(id);
            if (!user) throw new Error("User not found.");
            if (
